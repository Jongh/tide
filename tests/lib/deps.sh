...
#
# `read_deps`(위상정렬용 이름 방출)와 그 직접 보조 `strip_bom`·`dep_name`는 fleet·fleet-cycle
...
# 동일 로직의 참조 셸 절차로 고정한다.
#
# 의존성: `toposort`가 내부에서 `read_deps`를 호출한다. 따라서 하니스는 이 파일을 source한 뒤
# `tests/lib/toposort.sh`를 source해야 한다(read_deps 정의 제공). source 순서: discover → deps →
# toposort. 부수효과 없음 — 함수 정의만 담는다(픽스처·실행 0).
# 사용: . "$ROOT/tests/lib/discover.sh"; . "$ROOT/tests/lib/deps.sh"; . "$ROOT/tests/lib/toposort.sh"
#
# 추출 범위: 위상정렬에 필요한 `read_deps`(+`strip_bom`·`dep_name`)만 단일화한다. 계약 비교
# 전용 함수(`dep_required_*`·`semver_*`·`eval_op`·`check_contract` 등)는 추출 범위 밖 — 각 하니스
# 로컬에 남는다. (M17 BOM 처리·M20 전체 연산자 이름 보존을 정본으로 채택)

# 선두 UTF-8 BOM(EF BB BF) 제거(M17). 이름 방출 전 .tide/deps 첫 줄에 적용한다.

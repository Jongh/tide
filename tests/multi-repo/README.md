...
**현재 케이스 수 (cases: 37)** — `run.sh` @ **dash**(우분투 `/bin/sh`) · `run.sh` @ **bash**(Git
...
| 9 | 선두 UTF-8 BOM 입력 (impl/release 각각) | 2 / 0 | 가드의 **선두 BOM 내성** (판정이 BOM에 좌우되지 않음) |
| 9b | **뭉갠 BOM**(가드가 형태로 아는 어느 것도 아닌 선두 바이트, impl/release 각각) | 2 / 0 | 가드의 **선두 잡음 내성**(M42) — 형태 열거가 통하지 않던 부류 |
| 9c | **파싱 불가 입력**(JSON 아님 · 첫 `{`가 명령 한가운데) + 쓰기 / 읽기 | 2 / 0 | **보수적 폴백이 원본을 본다**(M42 재작업 1) — 정규화가 폴백의 시야를 좁히면 여기서 과소 차단이 난다 |
| 9d | 폴백 루트(CPD)에 **비-release phase가 있는** 구성 + cwd=release 레포 (잡음 없음 / 뭉갠 BOM) | 0 / 0 | **cwd가 폴백을 이긴다** — 멀티 레포 격리 규약의 답이고, 잡음이 그 답을 바꾸지 못한다 |
...
### 지원 기동 맥락 (`run.ps1`, M38-T05 → M42에서 재판정)

`run.ps1`은 **어느 호스트에서 띄워도 같은 판정을 낸다**(M42). Git Bash 호스트 금지는 **해제됐고**,
러너는 여전히 첫 단언 전에 **자기 채널을 탐침**하되 **묻는 것이 달라졌다**.

- **원래 왜 갈렸나(M38 실측 — 이력)**: 이 하니스는 가드에 훅 입력을 **stdin으로** 넣고, 그때 가드는
  `[Console]::In`으로 읽었다. 그 디코딩은 **콘솔 입력 코드페이지**를 따르고 자식 프로세스는 그것을
  띄운 호스트에서 물려받는다 — 네이티브 PowerShell 호스트는 CP **65001**(BOM 3바이트가 UTF-8
  디코더에 흡수), Git Bash 호스트는 CP **949**라 `EF BB`가 한 글자(U+7664)가 되고 `BF`가 뒤따르는
  `{`까지 삼켜 **여는 중괄호 자체가 사라졌다**(M42-T01 재실측 — M38이 적은 것보다 한 단계 더
  나빴다). `ConvertFrom-Json`이 throw하고 `cwd`가 비어 폴백으로 **통과**해, 시나리오 9만 뒤집혀
  **29/1**이었다. 원인은 BOM도 `$OutputEncoding`도 아닌 **기동 맥락**이었다.
- **M42가 (a)를 택했다**: 가드가 stdin을 **원시 바이트로 읽고 UTF-8로 직접 디코드**하도록 바꿔
  **디코딩 축 자체를 없앴다**(그 다음 첫 `{` 앞은 무엇이든 버린다). M38이 (a)를 미룬 사유 둘은
  둘 다 소멸했다 — 훅 무접촉 불변은 M42가 해제했고, 러너가 **공유 콘솔의 코드페이지를 고쳐 쓰는**
  경로는 애초에 택하지 않았다(그것이 가드의 실제 취약성을 가린다는 판단은 지금도 유효하다).
- **실측(M42 — 이 하니스 기준, 재작업 1에서 재측정)**: Git Bash 호스트에서 `pwsh` 자식으로 돌려
  **37/0**. 같은 호스트·같은 하니스에 **M42 이전 가드**를 놓으면 **34/3**이고, 붉는 것은 시나리오
  9·9b(`got 0, want 2` — 선두 BOM·뭉갠 BOM에서 차단이 통과로 뒤집힌다)와 9d의 잡음 케이스
  (`got 2, want 0` — 잡음이 `cwd` 추출을 깨 폴백이 판정을 가로챈다)다. 즉 이 케이스들은 공허하지
  않고, 가드가 되돌아가면 **조용히 통과하는 대신 큰 소리로 실패한다**.
- **탐침은 남긴다(묻는 것만 바뀌었다)**: 러너는 같은 `Start-Process` stdin 경로로 `EF BB BF` + `{}`를
  보내 **바이트가 그대로 도착했는지**를 본다(예전에는 "선두가 가드가 아는 형태로 **디코드**됐는지"를
  봤다). 가드가 바이트를 읽는 이상 채널이 바이트를 흘리면 어떤 판정도 의미가 없기 때문이다. 지원
  밖이면 케이스를 하나도 돌리지 않고 `# UNSUPPORTED LAUNCH CONTEXT …` + 코드페이지 진단을 찍고
  **exit 1**한다 — 조용히 다른 판정을 내지 않는다. 탐침은 **행위 기반**이지 코드페이지
  화이트리스트가 아니다(`65001`을 하드코딩하면 오늘 정당하게 초록인 CI 레그를 붉게 만든다).

...
**대상 레포**: 골격 산출물은 대상 레포 루트에 만든다. 규약은 `docs/conventions.md`의 "멀티 레포 / 대상 레포" 절.
...
4. docs/conventions.md — 단계별 금지행위(impl/review는 git 금지), 상태 파일(.tide/phase)
   ...
   여기에 **리뷰 규율 두 가지를 요지 수준으로** 함께 적는다 — 신규 프로젝트의 규약이 이 둘을
   담은 채 출발하게 하기 위한 것이며, 전문을 옮겨 적지는 않는다(문서 복제 드리프트를 새로 만들지
   않는다. **상세 규약의 단일 원본은 설치된 tide 스킬**이고 프로젝트 규약은 그 사실을 가리키기만 한다):
   - **리뷰 검증 규율** — `/tide:review`는 판정을 만들 때 판정 후보를 **스스로 공격하는 반증 시도
     (refutation)** 를 거치고, 그 결과를 리뷰보고서의 계측 줄에 남긴다. 리뷰가 직접 고친 것은
     **재검증(re-verify)** 하며, 판정 `불가` 후 재실행은 **재작업 라운드(rework)** 로 센다.
   - **차단 등급 판례** — 가드·검증이 **공허하게 통과**하거나 문서·규약이 **집행력을 사실보다 넓게
     주장**하면 그 지적은 **차단** 등급이다(`vacuous-pass`). 판례 부류에 가깝다고 보면서 차단으로
     매기지 않기로 했다면 그 **부인을 기록**한다(`precedent-waiver`).
...
**멀티 레포 맥락 감지 힌트 (advisory, 가산)**: 위 골격 생성(및 진행 중 프로젝트면
`docs/project-context.md` 생성)을 모두 마친 뒤, 출력 **맨 끝에** 다음을 덧붙여줘. 이 힌트는
위 기존 동작(골격·`.gitignore`·project-context 생성)을 **바꾸지 않고 그 위에 가산**될 뿐이다.

- **세는 대상**: 현재 세션 위치(cwd)의 **직속 1단계 하위 디렉터리** 중 **자식 tide 레포**의
  개수를 센다. 자식 tide 레포 판별·숨김(`.`으로 시작) 디렉터리 제외 규칙은 재정의하지 말고
  `docs/conventions.md`의 "멀티 레포 오케스트레이션"(발견) 절을 그대로 따른다.
- **출력 조건**: 자식 tide 레포가 **2개 이상**이면 맨 끝에 한 줄만 advisory로 덧붙인다 —
  `여러 자식 tide 레포 N개 감지 — 교차 개요는 /tide:fleet`(N=발견 수). **2개 미만(0·1개)이면
  아무 것도 덧붙이지 않는다**(소음 0) — 단일 레포 루트에서 실행한 일반 경우가 여기에 해당한다.
- **advisory 불변**: 이 힌트는 안내일 뿐 fleet/cycle을 **자동 실행하지 않는다**. 자식 tide 레포가
  2개 미만인 모든 세션에서 kickoff 출력은 현행과 동일하다(하위 호환 가산).

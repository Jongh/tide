...
7. .tide/debug-session 존재 여부 — 있으면 **열린 debug 세션**이고 그 안의 한 줄이 활성 보고서 번호 N
   (파일 없으면 "없음" = 열린 세션 없음)
8. `docs/epics/E*.md` 중 `status: open`인 것 — 있으면 **열린 줄기**와 그 에픽의 「속한 마일스톤」
   마지막 항목을 한 줄로 보여준다. **에픽이 하나도 없으면 아무 것도 덧붙이지 않는다**(소음 0 —
   `/tide:fleet` 힌트가 자식 레포 2개 미만에서 침묵하는 것과 같은 형태). 규약의 단일 원본은
   `docs/conventions.md`의 "에픽 (방향) 층" 절. **읽기만 한다.**

...
5. **릴리즈 커버리지 체크 (advisory — 차단 아님)** — 마지막 릴리즈 태그 이후 변경된 실질 파일 중
   **어떤 미릴리즈 보고서도 설명하지 않는 것**을 드러낸다(tide를 거치지 않은 커밋이 리뷰·CHANGELOG
   없이 태그에 편승하는지 노출). `git tag`로 마지막 태그를 찾고(태그가 없으면 전체 추적 파일),
   `git diff --name-only <태그>..HEAD`로 변경 파일을 모은 뒤, bookkeeping(`docs/reports/*`·`docs/milestones/*`·
   ...
   중단하지 않는다** — 사용자가 강행/보완을 판단한다. git **읽기만** 쓴다(phase 무관 통과·가드 무수정).

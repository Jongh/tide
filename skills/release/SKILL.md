...
4. **게시 분기** (1에서 정한 모드로):
   ...
   - **`pr` 모드 (상태 인지)** — 기본 브랜치에 직접 push하지 않는다. 1에서 조회한 PR 상태로 분기한다
     ...
     - **merged → 마무리(finalize)**: 기본 브랜치를 머지 결과로 최신화(`git fetch`→checkout/`pull`) →
       **PR CI 확인**(`gh pr checks {번호}`로 머지된 릴리즈 PR의 체크를 조회해 **태그 전에** 결과를
       보고한다. 실패·미완료 잡이 있으면 목록을 보여주고 **사용자 확인을 받은 뒤** 진행 — 프리플라이트
       3과 같은 형태다. **새 게이트가 아니다**: 프리플라이트 게이트 ①②③의 집합·순서·판정 로직은
       불변이다. `gh` 부재·조회 실패면 **그 사실을 한 줄 알리고** 진행한다 — 조용히 건너뛰지 않는다.
       이 축이 왜 여기서만 확인되는지의 단일 원본은 `docs/conventions-release.md`의 "`pr` 모드" 절
       PR CI 확인 항목) →

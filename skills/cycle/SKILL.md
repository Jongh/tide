...
- 인자 없음 → 가장 큰 번호 마일스톤의 **보고서 상태**로 시작점을 정한다:
  ...
  - impl·review 보고서 모두 있음(직전 사이클 완료) → **milestone 단계부터** (새 마일스톤)
    — 직전 마일스톤의 **릴리즈 여부는 묻지 않는다.** 회고가 릴리즈를 따라잡았는지는 milestone 단계가
    그 스킬의 advisory로 알린다(막지 않는다). 순서 규칙의 단일 원본은
    `docs/conventions.md`의 "회고 후속 항목의 소비" 절.
...
3. **review**: `.tide/phase`=`review`. 전제조건 — `docs/reports/M{N}-impl.md` 존재.
   ...
   (`skills/review/template.md` 구조). 이 단계는 `/tide:review` 규칙을 그대로 따르며, 넘어오는 것은
   **갈래까지 포함한 절차 전체**다 — 반증 시도(refutation)는 서브에이전트 경로와 **폴백** 갈래를,
   in-review 수정 후 재검증은 수정 0건 · 재실행 · **환경상 불가** 갈래를 함께 가져오고, 판정 계측은
   조건 갈래 없이 매 리뷰에 붙는다(어느 반증 갈래로 돌았는지를 그 줄이 적는다). 체이닝이라고 한 갈래만
   남기거나 폴백·불가로 돈 사실의 기록을 생략하지 않는다
   (규약은 `docs/conventions.md`의 "리뷰 검증 규율" 절).
...
- **`continuous`**: 판정이 **`불가`면 impl을 다시 돌고 review로 온다.** **상한은 새로 만들지 않는다** —
  ...
  - **왕복 중에도 각 단계의 기존 규칙·전제조건은 그대로다** — 리뷰는 매 라운드 위 review 단계가 적은
    갈래 전체(반증의 폴백 · 재검증의 환경상 불가 포함)를 그대로 가져온다. 자율은 **누가 다음 호출을
    시작하는가**만 바꾼다.

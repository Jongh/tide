...
- **minor·불변**: 커맨드 **12종의 호출명·역할·산출물·git 행위**, `.tide/phase` 값 집합, **tide-guard의 차단/통과 판정·메시지·exit 코드**(이번에도 훅은 **주석 전용 변경**, 비주석 0줄), 프리플라이트 게이트, 보고서 필수 섹션, 판정 표기·계측 줄 형식이 전부 불변이다. minor인 이유는 리뷰 절차 변경과 `kickoff`의 `.gitignore` 가산이 **사용자 관찰 가능**하기 때문이다. 전 하니스를 **세 실행 환경**(POSIX · 5.1 · 7)에서 재실행해 **18회 전부 FAIL 0**(discover 85 · fleet 41 · fleet-cycle 23 · fleet-verify 29 · multi-repo 30 · site-includes 34) — 5.1에서 전 하니스가 통과한 것은 처음이다. 남은 미실측은 CI의 ubuntu 레그 하나이며(로컬에 Linux 부재), 이 릴리즈가 푸시돼야 처음 돈다.

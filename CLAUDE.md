...
- **CHANGELOG 릴리즈 노트는 `[start:notes]` 마커 *다음* 줄에 넣는다** — 같은 토큰이 바로 위 설명
  주석에도 있어 **첫 매치로 삽입하면 마커 구획 밖**에 들어가고, `tests/site-includes`는 앵커
  정규식을 쓰므로 **초록인 채 통과한다**(가드가 아니라 편집자가 빠지는 자리다).

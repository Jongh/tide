...
- **리뷰 검증 규율(M32 · M36에서 다섯 번째 규약 가산)**: `/tide:review`가 판정을 **만들어내는 절차**의 규약이다.
  ...
  라이브 도그푸딩 영역)
- **차단 등급 판례(M36)**: 리뷰의 **등급 판단**과 **수정 권한**을 규약이 정한다 — 가드·검증이
  **공허하게 통과**하거나 규약·문서가 **집행력을 사실보다 넓게 주장**하면 **차단**이고(`vacuous-pass`),
  차단 등급은 **리뷰가 고치지 않고** 판정 `불가` + impl 반환한다(권장·사소만 in-review 수정). 판례

...
- **상호참조 무결성(M34, M35에서 문서 집합으로 일반화)**: 다른 문서가 규약을 가리키는 **인용**이
  ...
  유지한다. 단일 원본은 `docs/conventions.md`의 "상호참조 무결성" 절
- **축 상태 주장 정합**(M42): 표가 **집행된다**고 적은 축을 산문이 **반대로** 적는 자리를 문다.
  ...
  쓰지 않은 모순 · 반대 방향(발화 조건이 없어 실측 불가). 단일 원본은 `docs/conventions.md`의
  "축 상태 주장 정합" 절, 집행은 `tests/discover` **Part I**
...
| 좁은 부류의 **in-review 수정 허용**(자기 유발 · 한 줄 · 기계 검증 가능) | `.tide/NEXT-SESSION.md` 6절 후보 5 | **이월(M43에서 미착수 — 사유 기록)** — M43은 등급 루브릭(도달 가능성)과 종료 조건을 먼저 손봤고, 이 후보는 **그 둘이 실제로 왕복을 줄이는지 관측한 뒤** 판단하는 것이 순서다(지금 셋을 한꺼번에 풀면 어느 것이 값을 했는지 분리되지 않는다). **범위가 겹친다** — 도달 가능성 가중이 이미 "자기 훼손으로만 도달하는 공허"를 권장으로 내려 이 후보가 노리던 왕복 일부를 흡수했다. 다음 사이클이 rework 추이를 보고 재검토한다 |

...
- **리뷰 검증 규율(M32 · M36에서 다섯 번째 규약 가산)**: `/tide:review`가 판정을 **만들어내는 절차**의 규약이다.
  아래 ①~④가 M32의 넷이고, **다섯 번째(차단 등급 판례, M36)는 분량 때문에 아래 별 항목으로 뒀다**. ① **반증
  ...
  재분해 권고를 적는다(**advisory·비차단** — 자동 중단·자동 재분해 없음). **M43이 같은 임계에
  종료 조건을 더했다** — 라운드 2 초과에서 리뷰는 남은 차단을 **잔여 위험 수용**으로 처분할 수 있다
  (`가능` + 수용 근거 블록. 판정 토큰은 늘어나지 않고, **사용자 실행 표면에 닿는 차단은 수용 대상이
  아니다**. 자동이 아니라 선택이며 근거 없는 수용은 규율 이탈). **절차 가산이라 보고서
  ...
  라이브 도그푸딩 영역). **M44가 이 절의 규칙에 ASCII 병기어를 요구한다** — 병기어가 없는 규칙은
  Part E가 결합할 대상이 없어 재서술처가 낡아도 초록이기 때문이다(M43이 더한 종료 조건·도달 가능성
  가중에 병기어가 0개였고, 그래서 종료 조건을 무조건으로 부정하는 재서술 세 자리가 통과했다).
  **병기어 명단과 결합 조합의 단일 선언처는 `tests/discover/README.md`의 Part E 표**다 — 이 문서는
  명단을 옮겨 적지 않는다(옮겨 적으면 그 자체가 복제 선언 하나다)
...
- **상호참조 무결성(M34, M35에서 문서 집합으로 일반화)**: 다른 문서가 규약을 가리키는 **인용**이
  ...
  문서**이고 `docs/milestones/*`·`docs/reports/*`는 역사 기록이라 제외된다. 같은 파일 **안의**
  자기참조는 M41의 **G4**가, **규약 집합 밖 문서**(`skills/*/SKILL.md`·`docs/*.md`)를 가리킨 인용은
  M42의 **G5~G7**이 문다 — 후자는 대조를 **그 파일 자신의 앵커**로 하되 **골격의 `의` 조사**를
  요구하고(없으면 그 인용부호가 그 경로를 가리킨다는 근거가 없다), 이름 **유일성은 규약 집합에만**
  유지한다. **M44가 하위 인덱스를 금지한다** — 절보다 좁은 자리를 가리킬 때 **열거 번호가 아니라
  항목의 이름**을 적는다(번호는 열거가 바뀌면 조용히 다른 것을 가리키고, 항목이 닫히면 대상이 사라진다).
  집행은 **G8**(인용 줄에서 절 표지 뒤 열거 번호 0건 · 무는 문자 집합 U+2460~U+2487을 두 러너가 각자
  자기 단언 · 살아 있는 사이트가 0건이라 **금지형**이고 추출 positive-control과 픽스처 통제 둘이 공허를
  막는다). **이름으로 가리킨 항목의 실재는 대조하지 않는다**(실측: 사이트 5건 · 관측 결함 0건).
  단일 원본은 `docs/conventions.md`의 "상호참조 무결성" 절

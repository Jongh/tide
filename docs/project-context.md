...
- **신규 미실측 ④ 반증 패스의 서브에이전트 디스패치 경로**. 외부 프로젝트는 2회 모두 **폴백**으로 돌았다
  (세션 도구 인벤토리에 Agent/Task 호출 0건 — 보고서의 "Agent 도구 불가 → 폴백" 신고가 실제 제약과 일치했다).
  즉 반증 패스는 **디스패치 없는 환경에서만** 실증됐고, 서브에이전트 경로는 아직 라이브로 돈 적이 없다.

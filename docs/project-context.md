...
**라이브 도그푸딩 현황(2026-07-29 갱신)**: **gh 게시·`pr` finalize는 이미 M28(v2.6.0)에서 닫혔다** —
v2.6.0을 `pr` 모드로 내며 생성→머지→finalize(태그·릴리즈·브랜치 정리)를 실증했고, 이후 v2.7.0~v2.10.0도
전부 `pr` 모드로 나갔다(릴리즈 PR #1~#8). 위 M30 문단이 한동안 이 항목을 미반영으로 적고 있었으나
**직전 회고(v2.6.0 시점)가 이미 반영으로 기록**한 사실과 어긋난 서술이었고, M33에서 정정했다. 현재 남는
라이브 도그푸딩 미반영은 ① **M31 두 advisory의 런타임 발화**(릴리즈 커버리지 체크·마일스톤 번호
사전경고) ② **다인·혼성 팀 협업 사례** ③ **M32 반증 패스의 설치본 기준 강제 실증**(v2.10.0 설치 후
다음 리뷰 — M32의 실증은 리뷰어가 레포의 새 규약을 자발적으로 선적용한 결과였다)이다.

...
- **테스트**: 자동화된 테스트 러너 없음. 검증 수단은 **플러그인 재설치 후 새 세션 드라이런/도그푸딩**
  ...
  **재검증 선언**(`re-verify` 전용 병기어 — `in-review`는 계측 줄과 이중 용도라 재검증 절 삭제를 못 잡았다)·
  ...
  붉는다**)가 모두 존재한다(양 셸 `run.sh`·`run.ps1`)·`tests/mutation`(케이스가 자기
...
| 한 사이클의 반환 **셋이 전부 같은 부류** — 기존 하니스가 공통으로 가진 통제를 새 하니스가 빠뜨림 | M47 리뷰 3회차 + M47-impl 후속 8 | **fix(M48)** — 세 라운드의 반환(1회차 자기 `cases:` 대조 부재 · 2회차 선언 줄 유일성 미단언 · 3회차 `ps1` 완주 가드 미보유)이 **하나도 다른 부류가 아니었고**, 그 대조를 그때까지 **사람이 매번** 했다. M48이 공통 통제 목록을 규약에 **선언 블록**으로 못박고(단일 원본은 `docs/conventions.md`의 "새 하니스가 갖출 공통 통제 (목록 대조 — 기계가 문다)" 절) `tests/discover` **Part K**가 그 블록을 읽어 하니스마다 대조한다 — 면제는 침묵이 아니라 선언이고, **하니스가 늘면 대상이 는다**. **선언 줄 유일성 단언은 목록에 넣지 않았다**(보유가 일곱 중 하나라 「공통」이 아니다 — 새 검사 체크리스트의 **선언 줄 유일성** 항목은 *새로 세우는 검사*에 거는 작성 규율이지 기존 하니스 소급 요구가 아니다). **남긴 경계 둘** — 하나는 무는 것이 **토큰의 존재**이지 그 통제가 실제로 동작하는가가 아니라는 것이고(그 층은 `tests/mutation`과 케이스별 되돌림 실측이 덮는다), 다른 하나는 **반례가 하나만 기록돼 있다**는 것이다: 완주 플래그만 반대 사실이 남아 있고(출처 `docs/reports/M47-review.md`) 나머지 둘은 재작업 전 상태가 커밋되지 않아 **사후 재현 불가**다. 수치의 단일 출처는 `docs/reports/M48-impl.md` |

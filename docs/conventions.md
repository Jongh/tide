...
기본 브랜치에 **직접 push하지 않는다**. **릴리즈 브랜치**(예: `release/v{버전}`)에 버전 범프·
CHANGELOG를 commit → 브랜치 push → `gh pr create`(제목 `Release v{버전}: …`, 본문 = CHANGELOG
발췌, 제외 용어 규약 준수)로 PR을 연다. **태그·GitHub 릴리즈는 PR 머지 후로 미룬다**(PR 머지는
사람 몫) — release는 **"PR 열림"까지** 하고 PR URL을 보고한 뒤 사용자에게 **핸드오프**한다.
직접 push 흐름과 갈라지는 유일한 모드다.

- **릴리즈 브랜치 충돌**: 릴리즈 브랜치가 이미 있으면 — 같은 버전의 진행 중 PR이면 **그 브랜치를

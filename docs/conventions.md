...
- **① 1층 — 가시성 (이번 마일스톤)**: 읽기 전용 fleet 개요 = 자식 레포 **발견** + **교차
  ...
  권장 순서만 제시한다.
- **② 2층 — 의존성/계약 선언**: 자식 레포 간 의존·계약을 선언하는 매니페스트를 도입해
  **순서 인식**(어느 레포를 먼저 처리해야 하는지)을 가능하게 한다.
...
**2~4층은 후속 마일스톤**이다 — 이번 마일스톤은 그 토대 위에 올릴 **1층(가시성)만** 세운다.

### 부수효과 분리 불변

멀티 레포 토대에서도 부수효과 분리 원칙(위 "사이클" 절)은 그대로다.

- 오케스트레이션은 **cross-repo `git commit/tag/push`를 자동화하지 않는다**. release는 항상
  **레포별 수동**(`/tide:release`)이며, tide-guard와 레포별 격리(위 "멀티 레포 / 대상 레포"
  절)가 그대로 적용된다.
- fleet은 **advisory만** 한다 — 순서를 **제안**할 뿐 **실행하지 않는다**. 강제·자동 집행
  없음.

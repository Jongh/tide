...
- tide 플러그인이 설치돼 12종 커맨드와 tide-guard hook이 활성화돼 있다.
...
`.tide/deps`는 레포의 **선언**이므로 **커밋한다**. 반면 `.tide/phase`와 `.tide/debug-session`은
로컬 상태라 gitignore다 — gitignore 범위는 `.tide/`가 아니라 **이 두 파일만**으로 좁혀 둔다.

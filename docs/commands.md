...
- **전제조건(프리플라이트)**: ① review 판정 "가능" ② 테스트 통과 ③ 워킹트리 확인.
  통과해야 git 작업을 시작합니다.
- **커버리지 체크(advisory)**: 프리플라이트에서 **이번 태그에 실릴 파일** 중 **어떤 미릴리즈
  보고서도 설명하지 않는 것**을 경고합니다(차단 아님 — tide를 거치지 않은 변경이 미검토·CHANGELOG
  누락으로 태그에 편승하는지 릴리즈 시점에 노출). 보는 범위는 **마지막 태그 이후의 커밋된 변경**과
...
- **릴리즈 뒤 회고 자동 실행**(`retro-after-release`): **이 실행에서 태그가 달렸으면** `.tide/phase`를
  `idle`로 되돌린 **뒤** `/tide:retro`를 자동으로 실행합니다(도는 분기 = push-only·`release` 모드의 게시
  성공 · 두 모드의 `unpublished-release` 종료 · `pr` 모드의 **마무리**). `pr` 모드의 **생성·대기·중단**
  에서는 돌지 않습니다 — 태그가 없기 때문입니다. 회고가 실패해도 **릴리즈는 성공으로 끝납니다**(릴리즈가
  성립한 뒤 도는 사후 절차 — `unpublished-release`와 같은 층의 판단).
...
- **행 키 발급**: 후속 표에 **새 행을 만들 때** `항목` 셀 머리에 행 키 `rf-{NN}`을 발급하고, 기존 행의
  키는 다시 매기지 않습니다(`retro-row-claimed`). 단일 원본은 `docs/conventions.md`의 "회고 후속 항목의 소비" 절.
- **호출**: 사용자가 직접 부르거나, `/tide:release`가 태그를 단 실행의 **마무리 뒤** 자동으로
  부릅니다(`retro-after-release`). 자동으로 불려도 아래 「금지」와 「쓰기가 열린 두 자리」는 그대로입니다
  — 자동 실행이 쓰기를 늘리지 않습니다.

...
- **회고 따라잡음 확인(advisory)**: 최상단 회고의 기준 버전과 `CHANGELOG.md` 최상단 버전이 다르면 한 줄 경고합니다
  (`retro-after-release` — 새 마일스톤은 회고가 따라잡은 뒤에 세운다). 마일스톤은 그대로 만들고, 회고 문서가
  없으면 조용합니다. 단일 원본은 `docs/conventions.md`의 "회고 후속 항목의 소비" 절.
...
- **전제조건(프리플라이트)**: ① review 판정 "가능" ② 테스트 통과 ③ 워킹트리 확인.
  통과해야 git 작업을 시작합니다.
- **마일스톤 릴리즈 표지**(`milestone-release-mark`): 버전 파일·CHANGELOG를 고치는 단계에서, **이번 태그에 실리는
  마일스톤 전부**(표지 도입 번호 이상 · impl 보고서 있음 · 표지 없음 — 최대 번호 하나가 아님)의 메타데이터에
  `- released: v{버전}` 한 줄을 남깁니다. 강행이어도 초안(impl 보고서 없음)에는 쓰지 않고, debug만 근거인 릴리즈는
  쓰지 않습니다. 태그 없는 옛 표지는 그 버전의 릴리즈 PR이 없거나 미머지로 닫혔으면 이번 버전으로 바꾸고, 머지됐거나
  열려 있으면 바꾸지 않고 그 버전을 먼저 끝내라고 멈춥니다. 표지 도입 번호는 설치본에서 물려받지 않고
  (`mark-since-derived`) 대상 레포의 선언, 없으면 가장 작은 표지 번호로 정하며, 표지가 하나도 없으면 impl 보고서가 있는
  가장 큰 번호 하나에만 쓰되 그 리뷰 보고서가 마지막 릴리즈 태그에 이미 있으면(이미 나감) 쓰지 않습니다(소급 표지 없음).
  회고 추적이 「이미 나간 마일스톤」을 가리는 신호입니다. 단일 원본은 `docs/conventions.md`의 "회고 후속 항목의 소비" 절.
